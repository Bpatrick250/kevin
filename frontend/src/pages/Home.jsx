...
const Home = () => {
  ...
  const fetchHomeData = async () => {
    setLoading(true);
    try {
      const [testimonialsRes, programsRes, eventsRes, statsRes] = await Promise.all([
        api.getTestimonials().catch(() => ({ data: [] })),
        api.getPrograms().catch(() => ({ data: [] })),
        api.getUpcomingEvents().catch(() => ({ data: [] })),
  ...
  const handleNewsletter = async (e) => {
    ...
    try {
      ...
      Swal.fire({ icon: 'success', title: 'Subscribed! 🎉', text: 'Thank you for joining our newsletter!', confirmButtonColor: '#22c55e', timer: 3000 });
      setEmail("");
    } catch (error) {
  ...
  return (
    <div className="rlg-home pt-16">
      <style>{`
        ...
        .stat-card { text-align: center; padding: 2rem; border-radius: 1rem; border: 1px solid #e5e7eb; transition: all 0.3s ease; background: white; }
        ...
        .section-title { font-family: 'Playfair Display', Georgia, serif; font-size: clamp(1.8rem, 3vw, 2.6rem); font-weight: 800; color: #111; margin-bottom: .75rem; text-align: center; }
      `}</style>
        ...
        <div className="rlg-container">
          <div className="hero-grid">
            <div className="fade-up">
              <div className="rlg-hero-badge" style={{ display: 'inline-flex', alignItems: 'center', gap: '.5rem', background: 'rgba(255,255,255,.12)', padding: '.4rem 1rem', borderRadius: '999px', marginBottom: '1rem' }}>
                <FontAwesomeIcon icon={faStarHalfAlt} /> <span>RLG RWANDA</span>
              </div>
            ...
            <div className="fade-up"><img src={heroImage} alt="Young Leaders" style={{ width: "100%", borderRadius: "1rem", boxShadow: "0 20px 40px rgba(0,0,0,0.2)" }} /></div>
          </div>
        </div>
      </section>
      ...
      <section className="stats-grid">
        <div className="stat-card"><div className="stat-number">{stats.totalUsers || 100}+</div><div>Youth Reached</div></div>
        <div className="stat-card"><div className="stat-number">{stats.totalBlogs || 10}+</div><div>Success Stories</div></div>
      ...
      <section style={{ padding: "5rem 0", background: "#fff" }}>
        <div className="rlg-container">
          ...
          <div className="programs-grid">
            {programsData.slice(0, 3).map((program, i) => (
              <div key={i} className="program-card fade-up">
                <div style={{ width: '4rem', height: '4rem', background: 'linear-gradient(135deg, #22c55e, #16a34a)', borderRadius: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1rem' }}>
                  <FontAwesomeIcon icon={faUserTie} style={{ color: 'white', fontSize: '1.5rem' }} />
                </div>
                <h3>{program.title}</h3>
        ...
        <section style={{ background: "#14532d", padding: "5rem 0", color: "white" }}>
          <div className="rlg-container">
            <div className="testimonial-card">
              ...
              <p style={{ fontSize: "1.1rem", lineHeight: 1.8 }}>{testimonialsData[currentTestimonial]?.content}</p>
              <div style={{ marginTop: "1.5rem" }}>
                <strong>{testimonialsData[currentTestimonial]?.name}</strong>
                <p style={{ color: "#6b7280", fontSize: "0.8rem" }}>{testimonialsData[currentTestimonial]?.role}</p>
              </div>
            </div>
          </div>
        </section>
      ...
      <section style={{ padding: "5rem 0" }}>
        <div className="rlg-container">
          <div className="newsletter">
            ...
            <form onSubmit={handleNewsletter} style={{ display: "flex", gap: "1rem", maxWidth: "500px", margin: "1rem auto 0", flexWrap: "wrap", justifyContent: "center" }}>
              <input type="email" placeholder="Your email address" value={email} onChange={(e) => setEmail(e.target.value)} style={{ flex: 1, padding: "0.8rem", borderRadius: "999px", border: "none", minWidth: "200px" }} />
